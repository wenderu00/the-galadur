...
import { tick, rescaleQueueForSpeedChange } from '@/features/game-engine/engine';
import {
...
export function useTickEngine(): void {
  ...
  useEffect(() => {
    ...
    if (oldSpeed !== speed) {
      if (oldSpeed > 0 && speed > 0) applySpeedChange(oldSpeed, speed);
